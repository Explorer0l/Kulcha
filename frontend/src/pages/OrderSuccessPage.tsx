  ...
  useEffect(() => {
    ...
    const syncOrderWithAdmin = async () => {
      if (lastOrder && lastOrder.restaurantId) {
        ...
        try {
          // Проверяем существует ли заказ уже в системе админа (по ID)
          // В реальном приложении здесь будет более надежная проверка
          if (localStorage.getItem(`order_synced_${lastOrder.id}`)) {
          ...
          setTimeout(async () => {
            try {
              const retryOrder = await addOrder({
                restaurantId: lastOrder.restaurantId,
                customer: customerName,
                date: lastOrder.date,
                amount: lastOrder.totalAmount,
